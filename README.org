...
CMS which is the =bbdb= v3 package. The packages are optional, and the
configuration will work without it and the CMS menus in the org-capture
...
press =C-c C-x C-i= and =C-c C-x C-i=. Times and it's sum will be logged in the
=LOGBOOK= drawer.
