...
that is both simple and practical, and that caters to exactly what I need for my
day-to-day tasks.

I have explored and personally used several GTD system, and found them to be
overly complex for my needs.

The configuration aims to have a practical and lightweight GTD system using only
vanilla Emacs without any unnecessary packages to be usable.

Custom packages like =bbdb= are handled using only the Emacs built-in
=locate-library= function. The packages are optional, and the configuration will
work without them.
...
| Habit            | Repeated tasks with a visual graph | TODO                | 2 to 4 days           | projects.org/Recurring/Habit   |
